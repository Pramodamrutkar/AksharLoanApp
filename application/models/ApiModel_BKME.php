...
class ApiModel extends CI_Model{
	...
	public function getStudentDatadetails(){
		...
		if($UserData == FALSE){
			...
			if($this->form_validation->run() == FALSE){
				...
				$sql = "SELECT s.*,p.*, IFNULL(ld.isKyc, 0)  FROM studentmaster as s INNER JOIN parentmaster p on p.parentID=s.parentID LEFT JOIN loandetails as ld on ld.studentID = s.studentID WHERE s.`studentID`=? and p.accessToken=?";
	...
	public function saveKycdetails(){
		...
		//$loanAvailabilty = $this->checkLoanavailability($schoolID,$postArray["accessToken"]);
		//if($loanAvailabilty){
			if($UserData == FALSE){
				$Status='false';
				$Message = "Invalid Access Token";
		...
		if($UserData == FALSE){
			...
			$Message = "Invalid Access Token";
				...
				foreach($result as $k => $v){
					$sqlemi = "SELECT e.emiID, e.studentID, e.emiDuedate, e.emiAmount, CONCAT(s.sfirstName,' ',s.slastName) as studentName FROM `studentmaster` as s INNER JOIN emischedule as e on e.studentID= s.studentID WHERE e.emiStatus=0 AND e.studentID=? ORDER BY  e.emiDuedate ASC LIMIT 2";
