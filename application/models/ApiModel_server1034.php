	...
	public function getStudentDatadetails(){
		...
		if($UserData == FALSE){
			...
			if($this->form_validation->run() == FALSE){
				...
				$sql = "SELECT s.*,p.*,ld.*, ld.isApproved as isKyc FROM studentmaster as s INNER JOIN parentmaster p on p.parentID=s.parentID LEFT JOIN loandetails as ld on ld.studentID = s.studentID WHERE s.`studentID`=? and p.accessToken=?";
			...
			if($query->num_rows() > 0) {
				...
				$factor = ($schoolFinance['processingFeesgst'] / 100);
				$ResultArray['finalprocessingFees']= ($factor * $schoolFinance['fees']) + $schoolFinance['fees']; 	
				$ResultArray['processingFeesgstPercent'] = $schoolFinance['processingFeesgst'];
				$ResultArray['gstAmount'] = ($factor * $schoolFinance['fees']);
